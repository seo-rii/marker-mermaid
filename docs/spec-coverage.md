...
| structured Marker VLM | 구현 | engine별 격리 source snapshot, enabled-type root contract와 Phase 1 nested record 후검증, Marker 1.10.2 schema reserve와 stock Ollama inline-schema 호환을 포함한 text/item/view budget, reconstruction-global evidence budget, structural quota/backfill, prompt-selected prior만의 게시 권한, durable budget notice; generic `ir` envelope와 나머지 유형의 discriminated schema는 후속 |
