...
| structured Marker VLM | 구현 | engine별 격리 source snapshot, enabled-type root contract와 Phase 1·stable State/Class/ER 및 모든 Phase 2 native/fallback nested record 후검증/prompt contract, C4 token/legacy `type`, Deployment/Component canonical relation/port, Use-case open relation label과 fixed Flowchart proxy 형, optional partial field/evidence와 원본 IR/extra metadata 보존, typed IR의 3-field envelope와 hook-free exact-built-in snapshot, candidate UTF-8 1 MB/escaped JSON 4 MB·observation 및 fused-output JSON 8 MB/64개 집계 예산, pipeline/fusion/accessibility/repair/sidecar sink 재검증, Marker 1.10.2 schema reserve와 stock Ollama inline-schema 호환을 포함한 request text/item/view budget, reconstruction-global evidence budget, structural quota/backfill, prompt-selected prior만의 게시 권한, durable budget notice; generic `ir` envelope와 나머지 later-phase discriminated schema는 후속 |
