...
| Phase 2 software types | 구현 | State/Class/ER/Requirement/Block native; Requirement/Block과 C4/Deployment/Component fallback은 strict nested extraction contract까지 구현했고 Use-case만 아직 root contract; C4와 Deployment/Component 자동 게시·평가 Scene은 bounded record/Architecture plan의 emitted identity·group·unlabeled topology를 사용하고 runtime 거부 시 nested Flowchart로 재시도, 특수 notation과 relation label은 typed IR에 보존하며 진단용 native C4는 publication metric에서 분리; Use-case는 bounded collision-free Actor/UseCase plan을 공유하는 Flowchart 명시 fallback |
