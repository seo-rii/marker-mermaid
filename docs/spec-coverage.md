...
| structured Marker VLM | 구현 | engine별 격리 source snapshot, enabled-type root contract와 Phase 1·stable State/Class/ER·모든 Phase 2 native/fallback·Phase 3 chart(Pie/XY/Quadrant/Sankey/Radar/Treemap/Venn) 및 planning Journey/Kanban/GitGraph nested record 후검증/prompt contract, chart finite JSON number와 closed token, planning strict score/order와 GitGraph closed token, C4 token/legacy `type`, Deployment/Component canonical relation/port, Use-case open relation label과 fixed Flowchart proxy 형, optional partial field/evidence와 원본 IR/extra metadata 보존, typed IR의 3-field envelope와 hook-free exact-built-in snapshot, candidate UTF-8 1 MB/escaped JSON 4 MB·observation 및 fused-output JSON 8 MB/64개 집계 예산, pipeline/fusion/accessibility/repair/sidecar sink 재검증, Marker 1.10.2 schema reserve와 stock Ollama inline-schema 호환을 포함한 request text/item/view budget, reconstruction-global evidence budget, structural quota/backfill, prompt-selected prior만의 게시 권한, durable budget notice; generic `ir` envelope와 Packet/Ishikawa/Wardley/Cynefin/TreeView/Event Modeling/ZenUML/Railroad/Organization/Data Lineage nested schema는 후속 |
...
| OCR recall | 구현 | bounded occurrence multisets, spatial/bbox-less dedup, structural/Gantt/Class/ER/Timeline/Journey/Kanban/GitGraph 및 emitted Architecture group·C4 fallback boundary/service·Requirement·Deployment/Component·Use-case·EventModeling·Wardley·ZenUML serializer-visible labels, non-emitted metadata 배제, invalid/error/over-budget review gate |
| numeric consistency | 구현 | source에 존재하는 숫자만 비교; Journey score도 독립 source gate 적용 |
...
nested contract, generic `ir` envelope의 discriminated extraction schema, 아직 fallback
